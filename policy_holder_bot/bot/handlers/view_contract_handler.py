...
async def view_contract_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        FAILURE_COUNTER.inc()
        logger.error(f'Error while viewing contract info: {str(e)}')
        await query.edit_message_text('Error while viewing contract info. Try again later', reply_markup=get_main_menu_keyboard())